...
workflow run_methylTag_minimap2 {
    ...
    input {
        ...
        String REF_NAME
        String SAMPLE_NAME
        String CONTAINER = "registry-vpc.miracle.ac.cn/gznl/meredith705_ont_methyl:latest"
        String MINIMAP2_ARGS="-y -x map-ont -a --eqx -k 17 -K 10g"
        String OUT_LABEL=""
        Int CORES = 16
        Int DISK = round(size(REF_FILE, 'G'))) + (10 * round(size(UNALIGNED_METHYL_BAM, 'G'))) + 10
        Int MEM = 50
    }
...
task fastqAlignAndSortBam {
    input {
        ...
        String ref_name
        String sample
        String in_container = "registry-vpc.miracle.ac.cn/gznl/meredith705_ont_methyl:latest"
        String in_args      = "-y -x map-ont -a --eqx -k 17 -K 10g"
        Int in_cores        = 16
        Int in_disk = round(size(ref_file, 'G')) + round(size(unaligned_methyl_bam, 'G')) + 10
        Int in_mem          = 100
    }
    command <<<
    set -eux -o pipefail
